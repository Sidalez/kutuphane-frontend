...
export default function AddBookPage() {
  ...
  // 3 farklı puan
  const [expectedRating, setExpectedRating] = useState(0); // Okunacak
  const [progressRating, setProgressRating] = useState(0); // Okunuyor
  const [finalRating, setFinalRating] = useState(0); // Okundu
  ...
  // 🔎 ISBN ARAMA – kendi Node API'ına gider (isbn-api)
  const handleSearchISBN = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:3001/api/isbn-search?isbn=${encodeURIComponent(
          cleanISBN
        )}`
      );
      ...
      if (!data.found) {
        setIsbnMessage(
          data.message ||
            "Bu ISBN için kitap bulunamadı. Bilgileri manuel girebilirsin."
        );
        return;
      }
      ...
      setIsbnMessage("Kitap bilgileri başarıyla alındı. Formu kontrol edebilirsin.");
    } catch (err: any) {
      console.error("ISBN search error:", err);
      setIsbnMessage(
        "ISBN bilgisi alınırken bir hata oluştu. Daha sonra tekrar dene veya bilgileri manuel gir."
      );
    } finally {
  ...
  // 📖 Durum değişince sayfa alanını yönet
  useEffect(() => {
    if (status === "OKUNACAK") {
      setPagesRead("");
    } else if (status === "OKUNDU") {
      // kitap bittiğinde okunan sayfa = toplam sayfa (varsa)
  ...
  // 📚 Raf ekleme (tek raf, tag gibi gösteriyoruz)
  const addShelf = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setError(null);
 // ✅ Validation: Sayfa sayısı zorunlu
  if (!totalPages || totalPages <= 0) {
    ...
    try {
      ...
      // Genel puan (şimdilik basit ortalama; ileride AI ile değiştirilebilir)
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <section className="rounded-2xl border border-orange-200/80 dark:border-orange-900/70 bg-gradient-to-r from-orange-50/90 via-amber-50/90 to-white/90 dark:from-slate-950/90 dark:via-slate-950/80 dark:to-slate-900/90 px-4 py-4 md:px-5 md:py-5 space-y-3">
        ...
        <p className="text-xs text-slate-600 dark:text-slate-400">
          ...
          numarasını gir. Arama, <strong>isbnsearch.org</strong> verilerini ve{" "}
          <strong>AbeBooks</strong> kapak resmini kullanır.
        </p>
        ...
        <div className="flex flex-col md:flex-row gap-3 items-stretch">
          <div className="flex-1">
            <input
              ...
              placeholder="Örn: 9789750738609 veya 978-975-07-3860-9"
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  placeholder="https://pictures.abebooks.com/isbn/9789750738609-us.jpg"
                />
              </div>
  ...
  />
        ...
        <section className="space-y-4">
          ...
          {/* Genel puan bilgisi (sadece OKUNDU iken göster) */}
          {status === "OKUNDU" && (
            <p className="text-[11px] text-slate-400">
              Kitap bittikten sonra beklenti, gidişat ve bitmiş puanlarının{" "}
              <span className="font-semibold">
                arka planda otomatik ortalaması
              </span>{" "}
              alınarak genel puan hesaplanır.
            </p>
          )}
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            {/* Bitiş tarihi sadece OKUNDU ise */}
            {status === "OKUNDU" && (
              <div>
                <label className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">
                  Bitiş tarihi
